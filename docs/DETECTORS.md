...
The "physics doesn't lie" tier. Slower, harder, but extremely convincing when it fires, and highly explainable, which matters enormously for a human adjuster.
...
That same ChatGPT export carries a 29 KB `caBX` chunk, which is a **C2PA manifest
signed by OpenAI**, asserting
`digitalSourceType: http://cv.iptc.org/newscodes/digitalsourcetype/trainedAlgorithmicMedia`
with `softwareAgent: gpt-image`. The image declares under signature that a model made
it, and the pipeline was reading straight past it.

`provenance.content_credentials` now reads and **verifies** that manifest against its
certificate chain, which is a different evidential standard to the rest of the metadata
tier: an EXIF timestamp is a string anyone can type, while a tampered C2PA manifest
fails verification rather than lying convincingly. Same file, before and after:

```
before   AUTO_CLEAR   P = 0.288
after    FLAG         P = 0.876
...
It is the only tier that can produce **positive evidence of authenticity**, too: a
manifest asserting `digitalCapture` scores *below* 0.5. Every other detector in this
project can only fail to find a problem.

**The limit is severe and measured.** `scripts/sweep_credentials.py`:

| condition | credentials survive |
|---|---|
| as supplied | 100% |
| re-saved as PNG | **0%** |
| re-encoded to JPEG q95 / q75 | **0%** |
| resized by a messaging app | **0%** |
| screenshotted | **0%** |

One save strips it. That is not a flaw in C2PA, it is what metadata is: the bytes are
not the pixels, and nothing binds them once the chunk is dropped. So the detector
abstains on absence rather than reporting innocence, because a stripped generated image
and an honest photograph that passed through WhatsApp are byte-for-byte
indistinguishable on this signal.

Which leaves the genuinely open case: **a generated image whose credentials have been
stripped.** No tier in this project catches that today. The VAE-reconstruction idea
below is the candidate, and it remains unbuilt and unmeasured.
